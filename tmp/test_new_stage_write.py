...
def _enqueue_snapshot(nova_id: str, bibcode: str, bibstem: str, doctype: str,
                      ...
                      priority: int, reason: str) -> Dict[str, Any]:
    """
    ...
    item = {
        ...
        "ads_snapshot_uri": _s3_uri(STAGING_BUCKET, ads_snapshot_key) if ads_snapshot_key else None,
        ...
        "gsi1_sk": f"{priority:03d}|{entry_day}|{pk}",
        "gsi2_pk": f"NOVA#{nova_id or 'UNKNOWN'}",
        "gsi2_sk": f"{status}|{priority:03d}|{updated_at}",
    }
    # Remove None attributes (Dynamo best practice)
    item = {k: v for k, v in item.items() if v is not None}

    # Attempt to create; if exists, attempt a conditional priority raise
    try:
        ...
        if e.response["Error"]["Code"] != "ConditionalCheckFailedException":
            raise
        # Already exists: try to lower priority number if new one is higher priority
        try:
            queue_table.update_item(
                Key={"pk": pk, "sk": sk},
                UpdateExpression="SET updated_at=:t, ads_snapshot_uri=:u, reason=:r, "
                                 "#p = :new_prio, gsi1_sk=:g1, gsi2_sk=:g2",
                ConditionExpression="attribute_not_exists(#p) OR #p > :new_prio",
                ExpressionAttributeNames={"#p": "priority"},
                ExpressionAttributeValues={
                    ":t": _now_iso(),
                    ":u": _s3_uri(STAGING_BUCKET, ads_snapshot_key) if ads_snapshot_key else None,
                    ":r": reason,
                    ":new_prio": int(priority),
                    ":g1": f"{priority:03d}|{entry_day}|{pk}",
                    ":g2": f"{status}|{priority:03d}|{_now_iso()}",
                },
                ReturnValues="UPDATED_NEW",
            )
            return {"enqueued": True, "created": False, "pk": pk, "sk": sk, "priority": priority, "updated_priority": True}
        except ClientError as ee:
            if ee.response["Error"]["Code"] == "ConditionalCheckFailedException":
                # existing priority is already higher (numerically lower)
                return {"enqueued": True, "created": False, "pk": pk, "sk": sk, "priority": priority, "updated_priority": False}
            raise

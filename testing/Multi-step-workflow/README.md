...
```bash
Multi-step-workflow$ sam build --use-container
```
...
```bash
Multi-step-workflow$ sam logs -n StockCheckerFunction --stack-name "multi-step-workflow" --tail
```
...
```bash
Multi-step-workflow$ pip install -r tests/requirements.txt --user
# unit test
Multi-step-workflow$ python -m pytest tests/unit -v
# integration test, requiring deploying the stack first.
# Create the env variable AWS_SAM_STACK_NAME with the name of the stack we are testing
Multi-step-workflow$ AWS_SAM_STACK_NAME="multi-step-workflow" python -m pytest tests/integration -v
```
...
```bash
sam delete --stack-name "multi-step-workflow"
```

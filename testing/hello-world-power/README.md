...
```bash
hello-world-power$ sam build --use-container
```
...
```bash
hello-world-power$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
hello-world-power$ sam local start-api
hello-world-power$ curl http://localhost:3000/
```
...
```bash
hello-world-power$ sam logs -n HelloWorldFunction --stack-name hello-world-power --tail
```
...
```bash
hello-world-power$ pip install -r tests/requirements.txt --user
# unit test
hello-world-power$ python -m pytest tests/unit -v
# integration test, requiring deploying the stack first.
# Create the env variable AWS_SAM_STACK_NAME with the name of the stack we are testing
hello-world-power$ AWS_SAM_STACK_NAME="hello-world-power" python -m pytest tests/integration -v
```
...
```bash
sam delete --stack-name "hello-world-power"
```

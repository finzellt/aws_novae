...
Version: 1.8.0 - Added mocked service integration and support for AWS SDK integration.
Version: 1.8.1 - Added support for streaming APIs and global services and fixed an issue where TaskSubmitted execution history events weren't created for mock callback Tasks.
Version: 1.9.0 - Added support for the StartExecution.sync:2 service integration.
Version: 1.9.1 - Fixed bug causing executions with boolean Choice Rules containing a "Comment" field to fail
Version: 1.10.0 - Added support for StartSyncExecution and updated SFNASLJson dependency
Version: 1.10.1 - Upgraded Jetty version
Version: 1.10.2 - Validate resource is activity or state machine for ListTagsForResource, TagResource, and UntagResource. Fix intrinsic function interpolation when both context object path and JSON path are used. Fix incorrect history type for request/response service integrations when task fails. Fix service list loading errors. Fix some choice comparators ungracefully handling non-string types.
Version: 1.11.0 - Added new Intrinsic Functions.
Version: 1.11.1 - Apply endpoint overrides to corresponding AWS SDK service integrations.
Version: 1.12.0 - Added support for new Intrinsic Functions.
Version: 1.12.1 - Updated the method for detecting changes to mock configuration files by using hashing instead of the last modified date.
Version: 1.12.2 - Refactor hashing of mock configuration files for change detection.
Version: 1.12.3 - Update dependencies, no functional changes.
Version: 1.13.0 - Fail State: ErrorPath & CausePath. Retry: MaxDelaySeconds & JitterStrategy. Bug fix: prevent non-Task states from reading mocks. Add Map State fields: ItemProcessor and ItemSelector.
Version: 1.13.1 - Upgrade Netty from 4.1.68 to 4.1.97.
Version: 1.13.2 - Upgrade Dagger from 2.11 to 2.45.x and fix incorrect deserialization of DynamoDB attribute value types.
Version: 1.13.3 - Updated precision in WaitState to use milliseconds instead of seconds
Version: 1.14.0 - Update list of supported AWS SDK integrations
Version: 2.0.0  - Upgrade JDK 8 to 17. No functional change other than major version dependency updates.

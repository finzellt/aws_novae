# src/ads_harvest_candidates/app.py
import logging
from typing import Any, Dict, List
from pydantic import ValidationError
from nova_schema.nova import Nova
from nova_schema.mapping.nova_mapping import merge_updates
...
ADS_FIELDS = [
    ...
    "date","entry_date","data","author_count","abstract","title"
]
...
def _ads_request(canonical: Dict[str, Any], rows: int = 2000) -> Dict[str, Any]:
    token = _get_ads_token()
    headers = {"Authorization": f"Bearer {token}", "Accept": "application/json"}
    names = [q for q in (_quote(n) for n in _collect_names(canonical)) if q]
    params = {"q": " OR ".join(names), "fl": ",".join(ADS_FIELDS), "rows": rows, "sort": "date asc"}
...
def handler(event, _ctx):
    ...
    try:
        ...
        logger.exception("ADS request failed")
        raise

    # Map to harvest candidates
    candidates: List[BiblioSource] = map_ads_response_to_harvest(raw, nova.model_dump(mode="json").get("nova_id"))
    try:
        serial_candidates = [m.model_dump(mode="json") for m in candidates]
    except Exception as e:
        logger.exception(f"Failed to serialize candidates; Error: {e}")
        raise
    # (Optional) Update canonical breadcrumbs (e.g., note we queried ADS)
    updates = {}
    if candidates:
        bibcodes = []
    ...
    #         nova = merge_updates(nova, updates=updates)
    #     except ValidationError as e:
    #         logger.exception("Validation failed after ADS updates to Nova")
